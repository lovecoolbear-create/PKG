...
> **最后更新**：2026-08-31
> **代码基线**：方法论文档以提交 `b12e3ad` 之后为准。后续新增提交：`5434731`(全流程复测+CDP自净) / `66b5b72`(校准闭环工具链) / `f4cd90f`(§10 Backlog 三「高」项) / `39b75ae`(五维偏差热力图)。以上均已推送至 origin/main（远端与本地一致，无需补推）。
...
| 分享链接 | ✅ | 路由 `/share/[token]`；**2026-08-31 端到端已验证**：`verify-frontend-report.ts` 走完整链路（建会话→改参数→跑分析→生成分享→浏览器打开 `/share/<token>` 断言五维齐全且 console 0 错误）+ `e2e-share-link.ts` 14 项 |
...
- **浏览器测试的两类假红（2026-08-31 合并）**：① **CDP 端口残留** —— 无头 Chrome 未退干净会继续占用 `--remote-debugging-port`，后续测试连上僵尸实例：页面正常、console 0 错误，但断言全红。三个浏览器脚本已内置 `scripts/lib/cdp-port.ts` 启动前 + 收尾自净；手工调试遇「全红但无报错」先查 `lsof -nP -iTCP:9333` / `9334` / `9337` / `9338` `-sTCP:LISTEN`。② **dev server 冷编译** —— 首次访问某路由时 Next 正在编译，页面还没渲染出断言文案，`test:frontend` 会报「缺文案」（实测首跑 9/12，重跑 12/12）。**两者都是环境态而非代码缺陷：遇到前端走查失败，先重跑一次再动手改代码。**
...
- **验证**：tsc 0 错；`tests/deviation.test.ts` 59/59；`npm run test:heatmap` 13/13；`npm test` 全量确定性套件全绿；api 62/62、frontend-flow 13/13、calibration-chain 50/50、frontend 12/12 无回归。
- **推送**：提交 `39b75ae`（含此前未推的 `f4cd90f`）已成功推到 origin/main，远端与本地一致。
- **测试波动教训**：首次跑 `test:frontend` 出现 3 条「缺文案」假红（`/analyze` `/vave` `/share/<无效token>`），重跑 12/12 全绿 —— 原因是 dev server 首次编译未就绪、页面还没渲染出断言文案。判定前端缺陷前必须先重跑一次排除冷编译。
...
- **① 离线降级空状态**（真缺口，已修）：`AiChatPanel` 原先的 `cfgOk` 只在「用户点了发送且配置不可用」时才置位，等于让用户先踩一次坑才看到提示。改为接入全局 `useAiReady` 状态：
  - `offline` / `unconfigured` / `disabled` 三态在消息区顶部渲染显式占位卡（状态说明 + 该状态下哪些功能不受影响 + 「AI 设置」入口），输入框禁用并切换提示文案；
  - `unknown` 与 `checking` 期间不判定为离线，避免首屏探测未回时误伤。
- **② 中栏对话↔右栏产出联动**（真缺口，已修）：原先切换工作页时 `setArtifact(null)` 粗暴清空右栏，导致**切走再切回右栏凭空变空，而中栏对话历史还在**，两侧口径不一致。改为：
  - 绑定键细化为 `analyze:<品类>` / `vave:<项目id>`（原先 analyze 只有一个桶，不同品类共用）；
  - 新增 `src/lib/ai-artifact-store.ts`，产出按绑定键分桶持久化，与对话历史 `ai_chat:<key>` 同键规则 —— 切走落盘、切回恢复、跨页不串味；
  - 产出附带 `round`（第 N 轮）与 `sourceLabel`（主信息源），右栏底部标注「基于第 N 轮「主源X」对话归纳」，一眼可查来源；归纳中右栏顶部显示「更新中」增量指示；
  - 旧桶 `ai_chat:analyze` 读穿兼容（只读取不回写），细化分桶不丢历史；
  - **卸载落盘只写非空值**：`beforeunload` 写 null 会把桶里已有产出抹掉（该坑由回归测试暴露）。
- **③ AI 入口收敛**：核实后确认代码早已是收敛形态（`/work` 三栏 = 顶栏状态 + 中栏底对话 + 右栏产出；悬浮状态灯在 `/work`、`/intro` 自动隐藏；`/ai` 已降级为跳转），仅清单未回写，已按实际状态标记完成。
- **测试**：`verify-frontend-flow.ts` 由 5 项扩到 13 项，新增三组确定性回归——
  1. 离线降级（注入 `ai_settings={provider:"disabled"}` 后必须显式占位 + 输入框禁用，不依赖本机 LM Studio 是否在跑）；
  2. 产出分桶（注入种子产出 → 归属正确 / 切到别的品类看不到 / 切回仍在 / 标注第 N 轮）；
  3. 副驾驶必须给出确定状态（离线占位或可提问，二者必居其一）。
  - **踩坑**：页面上有多个 textarea（步骤 2 自然语言解析框也是），按 `placeholder` 精确识别副驾驶输入框，取 `querySelector('textarea')` 第一个会命中表单框导致断言假通过。
- **误判与回退**：一度认定「分析页点左侧『新建成本分析』是死按钮」并做了 JSX 重构，核实时发现该按钮只在「项目中心」视图渲染，分析页左侧是步骤导航 —— 重构已完整回退，`WorkbenchClient.tsx` 保持最小 diff。教训已写入 §10。
- **回归全绿**：tsc 0；golden 11/11 + 确定性自检；recipe-coverage 5 维×11/11；guardrail 16/16；binding 28/28；kernel 19/19；unit-norm 14/14；nlp 43/43；full-flow 94/94；api 62/62；frontend 12/12；frontend-flow 13/13；frontend-report 通过；calibration-chain 50/50。
...
- **新增测试覆盖**：
  ...
  - `scripts/verify-frontend-flow.ts`（`npm run test:frontend-flow`）：模拟真实点击流「新建成本分析 → 选品类 → 下一步 → 进入步骤 2 表单」，验证向导状态切换不崩、无 console 错误。
...
- **修复的真实缺陷**：
  ...
  4. **彩盒/瓦楞表单「订单数量」单位与报告不一致**：表单里写 `unit: "个"`，报告按 `unitLabel()` 显示 `/只`。用户此前已确认统一为「只」，现把 `color-print-box.ts` 与 `corrugated-box.ts` 的 `quantity.unit` 由「个」改为「只」，全链路口径一致。
- **验证**：`tsc` 0 错；确定性套件全绿（golden 11/11、recipe-coverage 五维全配方、guardrail 16/16、binding 28/28、kernel 19/19、unit-norm 14/14、nlp 43/43、full-flow 94/94、label 5/5）；需 dev server 的 E2E 全绿（api 62/62、share 14/14、frontend 12/12、frontend-flow 5/5、frontend-report 通过、print-pdf 5/5）。
...
| 高 | 供应商报价拆解对比工作台 | ✅ **v1.5 已落地（2026-08-31）**：xlsx 结构化报价表自动识别品类+语义列映射+材质文本解析 → `/import/compare` 汇总页，除原「我方单只 vs 客户单价」差额外，新增**五维结构偏差热力图**（双模式：结构偏差/单只金额）；点击任意行可跳转 `/work` 预填参数。**扫描件/图片报价（PDF/PNG/JPG）的 AI 视觉抽取已于 2026-08-28 落地**（`/api/import/quote-scan`：pdf-to-img 逐页渲染 → `scan-extract.ts` 视觉模型抽成逐字表格矩阵 → 复用与 xlsx 完全相同的 `runImportPipeline` 后段 → 前端预览核对后进同一对比页）；**未验证项**：真实扫描件的抽取质量尚未用本机视觉模型跑过（此前仅以 mock 端点验证链路 + 无模型时的兜底提示），需拿到真实扫描件后现场验证 |
...
| 优先级 | 项 | 说明 |
|---|---|---|
| 高 | AI 入口收敛 | ✅ **已落地（2026-08-31 核实）**：`/work` 三栏即收敛形态——顶栏状态灯 + 中栏底部对话 + 右栏结构化产出；悬浮状态灯在 `/work`、`/intro` 自动隐藏（避免三处并存）；`/ai` 已降级为 `router.replace("/work")` 兼容跳转 |
| 高 | 中栏对话↔右栏产出联动机制 | ✅ **已落地（2026-08-31）**：绑定键细化为 `analyze:<品类>` / `vave:<项目id>`，对话历史（`ai_chat:<key>`）与右栏产出（`ai_artifact:<key>`）共用同一套桶——切走落盘不丢、切回恢复、跨页不串味（原先是「切换即清空」，切回右栏凭空变空而中栏对话还在）；产出标注「第 N 轮 · 主源 X」，归纳中右栏显示「更新中」；旧 `ai_chat:analyze` 桶读穿兼容，历史不丢失 |
| 高 | 离线降级空状态 | ✅ **已落地（2026-08-31）**：`AiChatPanel` 接入全局 AI 就绪状态，`offline/unconfigured/disabled` 三态在消息区顶部显式占位（说明 + 「AI 设置」入口），输入框禁用并改提示文案，不再等用户点完发送才弹 ⚠️；`unknown/checking` 不误判为离线 |
...
### 两视角交汇（最高价值闭环）
- **供应商报价拆解对比**：v1.5 已落地（2026-08-31）——xlsx 结构化报价单作为新信息源（对比源），经 `/import/customer-quote` 自动映射后 `/import/compare` 产出「我方引擎 vs 客户报价」差额/毛利率对照 + **五维结构偏差热力图**，两条入口已合流（xlsx 走 `/api/import/customer-quote`，PDF/图片走 `/api/import/quote-scan` 视觉抽取，后段共用 `runImportPipeline`），与 VAVE 知识沉淀闭环方向一致。后续：① **用真实扫描件验证视觉抽取质量**（沙箱无视觉模型，此前仅 mock 验证链路）；② 新品类列映射注册。

### 2026-08-31 核实记录：三个「高」项的实际状态
评审清单写于 08-27，之后工作台持续改进，逐条核实结果：
- **AI 入口收敛**：代码早已是收敛形态，只是清单未回写。已按实际状态标记 ✅。
- **对话↔产出联动**：真实缺口在「产出不跟随工作页」——切换时 `setArtifact(null)` 粗暴清空，导致切回右栏空白而中栏对话仍在。改为按绑定键分桶持久化后口径一致，并补了三项确定性回归（归属 / 不串味 / 可恢复）。
- **离线降级空状态**：原先 `cfgOk` 只在「点发送且配置不可用」时才置位，等于让用户先踩一次坑。改为接入全局就绪状态进页面即判定。
- **本轮误判（记录以防重犯）**：一度认定「分析页点左侧『新建成本分析』是死按钮」，实则该按钮只渲染在「项目中心」视图（`LeftNav` 的 `activeView === "none"` 分支），分析页左侧是步骤导航。相关 JSX 重构已回退。**教训：断言 UI 缺陷前先确认控件在该视图是否真的渲染出来。**
- **遗留观察（非缺陷）**：VAVE 视图左侧无「新建成本分析」入口，需先返回项目中心。属设计取舍，未改动。

...
> **2026-08-31 最快的录入方式**：打开 `/calibration-intake`，用「批量导入」下载 xlsx 模板填写后上传（或从 Excel 直接粘贴），逐行校验后一键导入；攒够一批点页面上的「跑一轮校准」即可，不必手写 JSON、不必开终端。
> 下方字段字典是页面表单背后的落盘格式，排查数据问题时查阅。
